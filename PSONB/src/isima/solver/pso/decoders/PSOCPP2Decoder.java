 ...
 * @author onio
 */
public class PSOCPP2Decoder implements PSOPositionDecoder {
    ...
    public static final int N_DIMS = RULE_REF + 1;
    
    
    private final   Criterion   criterion;
    
    public PSOCPP2Decoder(Criterion inCriterion) throws Exception
    {
    ...
    public PSOPositionDecoder buildCopy() throws Exception {
        return new PSOCPP2Decoder(this.criterion);
    }
...
//            ppSolution.getSteps()++;
        ...
        step = (int)Math.round((PSOConstants.AUTO_MAX_STEPS - PSOConstants.AUTO_MIN_STEPS)* (pos[STEP_REF1] / (PSOConstants.POS_MAX - PSOConstants.POS_MIN)));
        
        if (step <= PSOConstants.AUTO_MIN_STEPS)
            step = PSOConstants.AUTO_MIN_STEPS;
        else if (step >= PSOConstants.AUTO_MAX_STEPS)
    ...
    public void copyContentFrom(PSOPositionDecoder inDecoder) throws Exception {
        //criterion.copyContentFrom(((PSOCPPDecoder)inDecoder).criterion);
    }

    @Override
    public String toDescString() {
        return ("Classic CPP Decoder");
    }
}
